...
CREATE OR REPLACE VIEW v_paidout AS
  ...
  UNION ALL
  (SELECT la.id AS loan_app_id, la.amount::numeric AS la_amount, p1.payment_date::date AS payment_date, coalesce(0.00)::numeric AS bank_sum, coalesce(p1.sum, 0.00)::numeric AS crm_sum,
          coalesce(p1.sum,0.00)::numeric AS total, coalesce(p1.bank,'crm_restructured') AS bank, la.loan_office AS loan_office, coalesce(p1.type,'restructure') AS type
   FROM loan_application la
     LEFT OUTER JOIN restructure_payment p1 ON p1.loan_app_id=la.id
